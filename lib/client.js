		...
		*/
		function toSnapshotView(list) {
			...
			for (const [id, row] of Object.entries(list.byId)) sessions[id] = {
				...
				...row.pendingInteraction === void 0 ? {} : { pendingInteraction: row.pendingInteraction }
			};
			const jobs = {};
		...
		*/
		function diffCompletions(prev, next) {
			...
			for (const [sessionId, row] of Object.entries(next.sessions)) {
				...
				if (before.pendingInteraction === void 0 && row.pendingInteraction !== void 0) events.push({
					kind: "review",
					sessionId,
		...
		*/
		function apply(ctx) {
			...
			const syncErrorWatchers = () => {
				...
				for (const [id, unsub] of [...errorUnsubs]) if (!ids.has(id)) {
					unsub();
				...
				for (const id of ids) {
					...
					const onSnapshot = () => {
						...
						if (before !== void 0 && before === null && err !== null) {
							const cfg = getSettings();
